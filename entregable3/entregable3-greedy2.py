...
for inici in grafoPrim.V:
    ...
    if (j==0):
        ...
        for i in range(len(solucio)-1):
            mejorDistancia+=distancia(vertices[solucio[i]],vertices[solucio[i+1]])
        mejorDistancia+=distancia(vertices[solucio[len(solucio)-1]],vertices[solucio[0]])
        solucion=solucio
    
    else:
        for i in range(len(solucio)-1):
            distan+=distancia(vertices[solucio[i]],vertices[solucio[i+1]])
        distan+=distancia(vertices[solucio[len(solucio)-1]],vertices[solucio[0]])
